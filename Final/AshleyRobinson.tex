...
Targeted at medical images the system follows standard image processing conventions and contains a sub-module that provides confidence information to domain expert.  
...
Sequential floating searching is used for feature selection when using a support vector machine for classification.
...
The problem is to be approached by considering the general cases so concentration is not applied to a specific area of the human body.
...
Dosages of radiation, cost and professionals time are all reasons to reduce the number of required patient scans.
Throughly inspecting any data gathered is a constructive method of scan reduction.
The objective is to provide a recommendation for a tool to be implemented to assist in medical practitioner diagnosis.  
...
A standard medical X-ray is tuned to produce and image of human bone.
...
Ultrasound is a method of image acquisition with the advantage of a small computational processing overhead allowing internal organs to be viewed in real-time which is beneficial because it allows practitioners to view internal physical movement. 
...
This data is typically stored as a series of 2D images which are slices of a scan.
An image viewer will build 3D images as and when required using these slices.
...
Software provides interactive images as shown in Fig.~\ref{fig:3d} which allows the user to move three planes to gain an impression of the region.
...
Figure~\ref{fig:ex} contains examples of identified tumours in different organs.
Tumours in brain (\ref{fig:exBrain}) and lung (\ref{fig:exLiver}) images are far simpler to identify than the liver image (\ref{fig:exLiver}).
...
Pre-processing methods are used to enhance the image captured end ensure it is correctly formatted for further processing.
...
An image histogram is a mapping from an image to a 2D plot of the frequency of voxel intensity.
This is possible because voxels only have a discrete value of intensity within a fixed range.
A histogram contains information on the type of material seen in the image, if it can be determined from intensity, and how much of that material is used to form the entire image.
Image enhancement can be performed using point operations with information from the histogram.
This is a very fast method of image enhancement as it is direct voxel-to-voxel mapping, as shown in Eq.~\eqref{eqn:point}. 
...
Eq.~\eqref{eqn:same} provides the same result as Eq.~\eqref{eqn:kernel} as describeded using Eq.~\eqref{eqn:dft}.
...
\subsection{Generalised Hough Transform (GHT)}

The GHT has been successfully applied to 3D images but as the shape of tumours are often unknown fitting a shape becomes redundant. 
Some tumours, Fig.~\ref{fig:exBrain}, grow spherically so the ability to fit a sphere using the GHT may be a useful feature for classification.

\subsection{Active Contours}

The active contour model is a methods for flexible shape extraction.
...
Supervised and unsupervised techniques for classification are considered even though a datasets have not yet been sourced.
...
SVMs have been successfully applied to breast cancer diagnosis in~\cite{xiufeng13svm}
...
AdaBoost is the most common implementation of a boosting algorithm~\cite{freund99boost} and has been successfully applied to brain tumour classification in~\cite{islam13multi}.
