...
Human supervised deployment is considered, after further testing, whether stand-alone or as an extension to an existing medical review software package.
...
Rather than trying to classify lung or brain tumours individually the goal is to consider what features tumours have in common and given an entire image of a human body is it possible to classify the presence of a tumour in any given section whilst using the same approach?
...
Available literature often considers 3D techniques as a simple extensions of 2D implementations but in practice this is not the case and for this reason an emphasis is placed on locating research in practical volumetric analysis with good results.   
...
Confidence information at the very least will be a percentage on the strength of the detection therefore multiple tumours will produce a stronger response. 
...
\begin{equation}
	\textbf{I}' = \mathcal{F}^{-1}(\mathcal{F}(\textbf{I})\times\mathcal{F}(\textbf{H})) = \mathcal{F}^{-1}(\textbf{I}*\textbf{H})		% Arrow doesn't work, implied matrices?
	\label{eqn:same} 
\end{equation}
...
The active contour model is a method for flexible shape extraction and has been throughly trialled in 2D~\cite{nixon02feature}.
...
These are not pure active contours and emulate behave by segmenting 2D dimensional regions then using volumetric growth algorithms to actively converge upon shapes for extraction.
...
Pure active contours could improve performance and current research is considered to expand dimensionality.
...
This research review has provided a recommendation for population of a convectional image processing architecture ending with standard machine learning approach.
Volumetric techniques for pre-processing and feature extraction have been reviewed extensively. 
Cutting edge implementations have been considered for anisotropic diffusion along with active surface contours which have also been expanded.
