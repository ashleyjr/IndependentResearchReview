...
The Fourier transform can be extended to 3D images.
A Discrete Fourier Transform (DFT) for a $N$$\times$$N$$\times$$N$ voxel image is held in Eq.~\ref{eqn:dft}.
A spatial filtering performs convolution then once in the frequency domain the same effects can be achieved using multiplication.
...
\begin{equation}
	\textbf{FP}_{u,v,w} = \frac{1}{N^{\frac{3}{2}}} \sum\limits_{x=0}^{N-1}\sum\limits_{y=0}^{N-1}\sum\limits_{z=0}^{N-1}\textbf{P}_{x,y,z}e^{-j\frac{2\pi}{N}(ux + vy + wz)}
	\label{eqn:dft} 
\end{equation}
