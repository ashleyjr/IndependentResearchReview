...
\caption{Voxel Neighbourhoods. Taken from~\cite{lohmann1998volumetric}.}
...
Eq.~\eqref{eqn:ft} describes a Fourier pair, forward and inverse respectively, and Eq.~\eqref{eqn:same} provides the same result as Eq.~\eqref{eqn:kernel}. 

\begin{equation}
	F_{u,v,w} = \frac{1}{N^{\frac{3}{2}}} \sum\limits_{x=-\frac{N}{2}}^{\frac{N}{2}-1}\sum\limits_{y=-\frac{N}{2}}^{\frac{N}{2}-1}\sum\limits_{z=-\frac{N}{2}}^{\frac{N}{2}-1}P_{x,y,z}e^{-j\frac{2\pi}{N}(ux + vy + wz)}
	\label{eqn:dft} 
\end{equation}
